    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.token && response.data.data.user) {
                ...
                logger.error("Login failed: Invalid response structure.", response.data);
                setError('Login failed. Unexpected response from server.');
            }
        } catch (err) {
            const errorMessage = err.response?.data?.message || 'Login failed. Please check credentials or network.';
        ...
        <main className={styles.container}>
            ...
            <section className={styles.formContainer}>
                ...
                <form className={styles.form} onSubmit={handleSubmit}>
                    <InputField
                        label="Email" name="email" type="email" placeholder="ex. email@domain.com"
                        value={formData.email} onChange={handleChange} required
                    />
                    <InputField
                        label="Password" name="password" type="password" placeholder="Enter password"
                        value={formData.password} onChange={handleChange} icon="password" required // Use icon prop
                    />
