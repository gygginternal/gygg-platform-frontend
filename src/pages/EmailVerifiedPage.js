...
function EmailVerifiedPage() {
    ...
    useEffect(() => {
        ...
        const verifyTokenApiCall = async () => {
            try {
                logger.info("EmailVerifiedPage: Attempting email verification with token:", token);
                ...
                setTimeout(() => {
                    navigate('/login', { state: { message: "Email verified! Please log in." } }); // Pass success message to login page
                }, 3000); // 3 seconds delay

            } catch (err) {
                logger.error("EmailVerifiedPage: Verification API call failed:", err.response?.data || err.message);
                setMessage(err.response?.data?.message || 'Email verification failed. The link may be invalid or expired. Please try resending the verification email.');
                setVerificationStatus('failed');
            }
        };
    ...
    return (
        <main className={styles.container}>
             <Link to="/" className={styles.logo}><img src="/gygg-logo.svg" alt="GYGG logo" width={100} height={60}/></Link>
