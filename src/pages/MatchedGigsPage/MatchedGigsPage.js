...
import MatchedGigCard from '../../components/MatchedGigsPage/MatchedGigCard'; // Assuming you have this component
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          'Failed to find matched gigs. Ensure your profile hobbies/preferences are set.'
      );
  ...
  return (
    <div className={styles.container}>
      ...
      <p className={styles.subtitle}>
        Showing open gigs posted by providers with similar hobbies or people
        preferences to yours.
      </p>
      <button
        onClick={() => fetchMatchedGigs(1)}
        disabled={loading}
      ...
      {matchedGigs.length > 0 ? (
        <ul className={styles.gigList}>
          ...
          {matchedGigs.map(gig => (
            <MatchedGigCard key={gig._id} gig={gig} />
          ))}
        </ul>
      ) : (
        !loading && (
          <p className={styles.noGigsMessage}>
            No matching open gigs found based on your profile preferences.
          </p>
        )
      )}
      ...
      {loading && currentPage > 1 && (
        <p className={styles.loadingMoreMessage}>Loading more gigs...</p>
      )}
