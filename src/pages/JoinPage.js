...
const RoleCard = ({ roleName, imageSrc, title, description, selected, onClick }) => (
    <div
        className={`${styles.card} ${selected ? styles.selected : ""}`}
        onClick={() => onClick(roleName)} // Pass roleName on click
        role="button" // Accessibility
        tabIndex={0} // Accessibility
        onKeyPress={(e) => { if (e.key === 'Enter' || e.key === ' ') onClick(roleName); }} // Accessibility
    >
...
function JoinPage() {
    ...
    const handleContinueClick = () => {
        ...
        navigate('/signup', { state: { selectedRole: selectedRole } });
    };
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.content}>
                ...
                <button className={styles.continueButton} onClick={handleContinueClick} disabled={!selectedRole}>
                    {/* Button text changes based on selection */}
                    {selectedRole ? `Sign Up as ${selectedRole === 'tasker' ? 'a Tasker' : 'a Provider'}` : 'Select a Role'}
                </button>
                {message && <p className={styles.message}>{message}</p>}
                 <p className={styles.loginText}>Already have an account? <Link to="/login" className={styles.link}>Log In</Link></p>
            </div>
        </div>
    );
}
