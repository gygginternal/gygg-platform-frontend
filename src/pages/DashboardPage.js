...
function UserProfileForm() {
    ...
    if (user) {
      setFormData({
        ...
        availability: user.availability
          ? { ...defaultAvailability, ...user.availability }
          : { ...defaultAvailability },
  ...
  return (
    ...
    >
      ...
      <div>
        <label htmlFor="peoplePreference">People Preference</label>
        <input
          type="text"
          id="peoplePreference"
...
function DashboardPage() {
  ...
  return (
    <div>
      {" "}
      {/* No card class here, apply to sections */}
      <div className="card">
        <h2>Dashboard</h2>
        <p>
          Welcome back,{" "}
        ...
        {isTasker && (
          <p>
            Rating: {user.rating?.toFixed(1)}⭐ ({user.ratingCount || 0}{" "}
            reviews)
          </p>
        )}
        {isTasker && user.ratePerHour > 0 && (
          <p>Hourly Rate: ${user.ratePerHour.toFixed(2)}</p>
        )}
      ...
      <StripeOnboarding />
      {/* Stripe onboarding still uses its own card */}
