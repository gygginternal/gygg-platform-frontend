...
function ProviderOnboardingPage() {
    ...
    const handleNavigation = (direction) => {
        ...
        if (direction === 'back') {
            ...
            if (currentStep === 1 && (!formData.firstName.trim() || !formData.lastName.trim() || !formData.address.postalCode.trim())) {
                 setError("First name, last name, and postal code are required for step 1."); return;
            }
    ...
    const handleSubmitOnboarding = async () => {
        ...
        if (formData.address) {
            Object.keys(formData.address).forEach(key => {
                if (formData.address[key]) { // Only append if value exists
        ...
        logger.debug("Submitting Provider onboarding data:", Object.fromEntries(payload.entries()));
        try {
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            case 1: // Basic Info (profilesetup1)
                return (
                    <>
                        <h1 className={styles.formTitle}>Tell us about yourself (Provider)</h1>
                        <p className={styles.formDescription}>This information helps Taskers understand who they might be working for.</p>
                        <div className={styles.formGrid}>
                         ...
                         <FormInput label="Postal code*" name="postalCode" value={formData.address.postalCode} onChange={(name, val)=>handleAddressChange(name,val)} required placeholder="e.g., A1B 2C3" />
                         <FormInput label="City (Optional)" name="city" value={formData.address.city} onChange={(name, val)=>handleAddressChange(name,val)} placeholder="e.g., Toronto"/>
                         <FormInput label="Province/State (Optional)" name="state" value={formData.address.state} onChange={(name, val)=>handleAddressChange(name,val)} placeholder="e.g., Ontario"/>
                         <FormInput label="Country (Optional)" name="country" value={formData.address.country} onChange={(name, val)=>handleAddressChange(name,val)} placeholder="e.g., Canada"/>
                    </>
                );
            case 2: // Personality/Preferences (profilesetup2)
                return (
                    <>
                        ...
                        <p className={styles.formDescription}>What kind of Taskers are you looking for?</p>
                        <PersonalityForm
                            peoplePreference={formData.peoplePreference}
                            onPreferenceChange={(name, value) => handleInputChange(name, value)} // Pass correct handler
            ...
            case 3: // Bio and Profile Picture (profilesetup3)
                return (
                    <>
                        ...
                        <p className={styles.formDescription}>Make your profile stand out.</p>
                        <BioAndPictureForm
                            ...
                            onProfileImageChange={(file) => setFormData(prev => ({ ...prev, profileImageFile: file }))}
    ...
    return (
        <div className={styles.pageContainer}> {/* Use a wrapper to apply background */}
            <div className={styles.onboardingContentWrapper}> {/* For centering and max-width */}
                 <div className={styles.logoTop}>
                     <img src="/gygg-logo.svg" alt="GYGG Logo" width={120} />
                 </div>
                <ProgressBar current={currentStep} total={TOTAL_PROVIDER_STEPS} label="Provider Profile Setup" />
                <main className={styles.formSection}> {/* Use styles from profilesetup1 */}
                ...
                <NavigationButtons
                    ...
                    canGoNext={
                        (currentStep === 1 && formData.firstName && formData.lastName && formData.address.postalCode) ||
