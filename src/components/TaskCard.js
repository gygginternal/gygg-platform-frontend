...
const Icon = ({ src, alt, className, width = 16, height = 16 }) => (
    <img src={src} alt={alt} className={className} width={width} height={height} onError={(e) => e.target.style.display='none'}/>
);

function TaskCard({ gig }) { // Accept a single 'gig' object prop from the backend
  const [isModalOpen, setIsModalOpen] = useState(false);
  const navigate = useNavigate();

  if (!gig || !gig.postedBy) {
    // Optional: Render a placeholder or null if gig data is incomplete
    return <article className={styles.taskCard}><p>Loading gig data...</p></article>;
  }

  // --- Data Mapping ---
  const image = gig.postedBy.profileImage || '/assets/default-profile.png'; // Use postedBy image or fallback
  const author = gig.postedBy.fullName || `${gig.postedBy.firstName} ${gig.postedBy.lastName}`;
  const title = gig.title;
  const rate = gig.ratePerHour ? `$${gig.ratePerHour}/hr` : (gig.cost ? `$${gig.cost} (fixed)` : 'N/A'); // Determine rate display
  const location = gig.location?.city || gig.location?.address || (gig.isRemote ? 'Remote' : 'Location N/A');
  const postedTime = gig.createdAt ? `Posted ${new Date(gig.createdAt).toLocaleDateString()}` : 'Recently'; // Format date

  const handleImageError = (e) => { e.target.src = '/assets/default-profile.png'; }; // Fallback image
  ...
  return (
    <>
      {/* Task Card */}
      <article className={styles.taskCard} onClick={openModal} role="button" tabIndex={0}>
        <img src={image} alt={`${author}'s profile`} className={styles.taskImage} width={155} height={182} onError={handleImageError}/>
        <div className={styles.taskContent}>
          <p className={styles.taskAuthor}>{author}</p>
           {/* Link title to detail page */}
           <Link to={`/gigs/${gig._id}`} className={styles.taskTitle} onClick={(e) => e.stopPropagation()}>
            {title}
          </Link>
          <p className={styles.taskRate}>{rate}</p>
          <div className={styles.taskDetails}>
            <div className={styles.taskLocation}>
              <Icon src="/assets/location.svg" className={styles.locationIcon} alt="Location" />
              <span>{location}</span>
            </div>
        ...
        <div className={styles.modalOverlay} onClick={closeModal}>
          ...
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h2>Gig Details</h2>
              <button className={styles.closeButton} onClick={closeWithButton} aria-label="Close modal">
