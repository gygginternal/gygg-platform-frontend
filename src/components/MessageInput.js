...
function MessageInput({ onSendMessage, disabled = false }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Parent component should handle displaying send errors
            console.error("Message send failed in child:", error);
        } finally {
            setIsSending(false);
        }
    };
    ...
    return (
        ...
        <form onSubmit={handleSubmit} className={styles.inputContainer}>
            ...
            <button type="button" className={styles.actionButton} disabled={disabled || isSending} aria-label="Attach file">
                 <img src="/attachment.svg" alt="Attach" className={styles.actionIcon} width={20} height={20}/> {/* Ensure icon in public */}
            </button>
            ...
            {/* Send Button */}
            <button type="submit" className={styles.actionButton} disabled={disabled || isSending || !messageText.trim()} aria-label="Send message">
