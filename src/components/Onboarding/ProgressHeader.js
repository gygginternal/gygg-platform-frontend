// src/components/Onboarding/ProgressHeader.js
                    ...
                    <div className={styles.navigation}>
                        {step > 1 && canGoBack ? ( // Show back button only if not on first step
                            <button className={styles.backButton} onClick={() => onNavigate('back')}>
                                <ArrowLeft size={20} style={{ marginRight: '5px' }} /> {/* Adjusted icon size */}
                                Back
                            </button>
                        ) : (
                            <div style={{width: '80px'}}></div> /* Placeholder for spacing if back button hidden */
                        )}
