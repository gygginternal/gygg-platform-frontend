...
function ChatHeader({ chatPartner }) { // Pass the other user object
    if (!chatPartner) {
        return ( // Placeholder or empty state when no chat is selected
    ...
    const name = chatPartner.fullName || `${chatPartner.firstName} ${chatPartner.lastName}`.trim();

    return (
        <header className={styles.header}>
            <img // Use standard img
                src={chatPartner.profileImage || "/default-profile.png"} // Use actual data or fallback
                alt={name}
