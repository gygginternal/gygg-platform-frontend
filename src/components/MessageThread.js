...
function MessageThread({ messages = [] }) {
    ...
    const handleImageError = (e) => { e.target.src = '/default-profile.png'; };

    if (!user) return <div className={styles.thread}><p>Loading user...</p></div>; // Need user to style messages
    ...
    return (
        <div className={styles.thread}>
            {messages.map((msg, index) => {
                ...
                const isContinuous = prevMessage && prevMessage.sender?._id === msg.sender?._id;

                return (
                    <article
                        key={msg._id || index}
                        className={`${styles.messageWrapper} ${isSender ? styles.sent : styles.received} ${isContinuous ? styles.continuous : ''}`}
                    >
                        ...
                        <div className={styles.messageContent}>
                             {!isContinuous && ( // Only show name for the first message in a group
                                <div className={styles.messageHeader}>
                                    <h3 className={styles.userName}>{msg.sender?.firstName || 'User'}</h3>
                                </div>
                             )}
                            <p className={styles.messageText}>{msg.content}</p>
                            {/* Show timestamp on hover or always */}
                             <time className={styles.timestamp}>{formatTimestamp(msg.timestamp || msg.createdAt)}</time>
                        </div>
                    </article>
                );
            })}
