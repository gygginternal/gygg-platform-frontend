...
const initialGigFormData = {
    ...
    gigDescription: "",
    isRemote: false,
    gigLocation: { address: "", city: "", state: "", postalCode: "", country: "" },
    skillsRequired: [], // Skills provider is looking for in a tasker
};

function GigCreateForm({ onGigCreated }) { // Callback for when gig is successfully created
    ...
    const handleCheckboxChange = (name, checked) => {
         setFormData(prev => ({ ...prev, [name]: checked })); setError('');
    };
    const handleLocationChange = (fieldName, value) => {
        setFormData(prev => ({ ...prev, gigLocation: { ...prev.gigLocation, [fieldName]: value } })); setError('');
    };
    ...
    const handleStepNavigation = (direction) => {
        ...
        if (direction === 'back') {
            ...
            if (currentStep === 1 && (!formData.gigTitle.trim() || !formData.gigCategory)) {
                 setError("Gig title and category are required."); return;
            }
            if (currentStep === 2 && (!formData.gigDescription.trim() ||
                ...
                (formData.gigPaymentType === 'hourly' && (!formData.gigRatePerHour || parseFloat(formData.gigRatePerHour) <= 0)))) {
                 setError("Gig description and valid budget details are required."); return;
            }
    ...
    const handleGigSubmit = async () => {
        if (!user) { setError("You must be logged in to post a gig."); return; }
        ...
        const payload = {
            ...
            location: (formData.isRemote || Object.values(formData.gigLocation).every(val => !val)) ? undefined : formData.gigLocation,
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            case 1:
                return <GigPostTimelineCategory formData={formData} onInputChange={handleInputChange} />;
            case 2:
                return (
                    <>
                        <GigPostDetailsBudget formData={formData} onInputChange={handleInputChange} />
                        <InputField label="Skills you're looking for in a Tasker (comma-separated, optional)" name="skillsRequired" type="textarea" value={Array.isArray(formData.skillsRequired) ? formData.skillsRequired.join(', ') : formData.skillsRequired} onChange={(name, value) => handleInputChange(name, value.split(',').map(s=>s.trim()))} rows={3}/>
                    </>
                );
            case 3:
    ...
    } else if (currentStep === 2 && (!formData.gigDescription.trim() ||
        (formData.gigPaymentType === 'fixed' && (!formData.gigCost || parseFloat(formData.gigCost) <= 0)) ||
        (formData.gigPaymentType === 'hourly' && (!formData.gigRatePerHour || parseFloat(formData.gigRatePerHour) <= 0)))) {
        canGoNextForCurrentStep = false;
    }
