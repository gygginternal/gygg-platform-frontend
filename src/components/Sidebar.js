...
const Icon = ({ src, alt, className }) => (
     <img src={src} alt={alt} width={24} height={24} className={className} onError={(e) => e.target.style.display='none'}/>
 );
...
function Sidebar({ isOpen, toggleSidebar }) {
  ...
  const navItems = [
      ...
      { key: 'messages', path: '/messages', icon: '/assets/messages.svg', text: 'Messages' }, // Placeholder path
      { key: 'contracts', path: '/gig-history', icon: '/assets/clock.svg', text: 'Task History' }, // Use your history path
      { key: 'gigs', path: '/gigs', icon: '/assets/briefcase.svg', text: 'Gigs' }, // Path for all gigs
      // Add other items as needed (Settings, Profile etc.)
  ];
