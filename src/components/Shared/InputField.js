...
function InputField({
  ...
  const [countryCode, setCountryCode] = useState(isPhoneInput ? (value?.match(/^\+\d{1,3}/)?.[0] || "+1") : "+1");
  const [localNumber, setLocalNumber] = useState(
    isPhoneInput ? (value?.substring(countryCode.length).replace(/\D/g, "") || "") : ""
  );
  ...
  useEffect(() => {
    if (isPhoneInput) {
      ...
      const newLocalNumber = value?.substring(newCountryCode.length).replace(/\D/g, "") || "";
  ...
  } else if (icon === "password" || name === "password" || name === "passwordConfirm" || name === "currentPassword" || name === "newPassword") {
      ...
      <div className={styles.inputContainer}>
        ...
        {type === "textarea" ? (
          ...
          <input
            ...
            required={required}
            disabled={disabled}
            autoComplete={name.includes('password') ? 'new-password' : (type === 'email' ? 'email' : 'on')}
