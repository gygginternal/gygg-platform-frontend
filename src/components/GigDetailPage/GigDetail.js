...
import { GigApplications } from '../Applications/GigApplications';
...
const ConfirmButton = ({ clientSecret }) => {
  ...
  const handleConfirmPayment = async () => {
    ...
    if (error) {
      ...
      // Optionally use logger.error or handle error appropriately
    }
  };
...
function GigDetailPage() {
  ...
  const fetchPaymentDetails = useCallback(async contractId => {
    try {
      const response = await apiClient.get(
  ...
  }, [gigId, user, fetchPaymentDetails]); // Add fetchPaymentDetails to dependencies
