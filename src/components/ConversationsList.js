...
function ConversationsList({ onSelectConversation, selectedContractId }) {
    ...
    useEffect(() => {
        const fetchConversations = async () => {
            if (!user) { setLoading(false); return; } // Don't fetch if no user
            ...
            try {
                ...
                logger.debug("Fetched conversations:", response.data.data.conversations);
            } catch (err) {
                logger.error("Error fetching conversations:", err.response?.data || err.message);
    ...
    const handleImageError = (e) => { e.target.src = '/default-profile.png'; }; // Fallback in public folder

    if (loading) return <aside className={styles.sidebar}><p>Loading conversations...</p></aside>;
    if (error) return <aside className={styles.sidebar}><p className="error-message">{error}</p></aside>;

    return (
        <aside className={styles.sidebar}>
            <div className={styles.header}> {/* Added a header for the list */}
                <h3>Messages</h3>
            ...
            <div className={styles.conversationsList}>
                {conversations.length > 0 ? (
                    conversations.map((convo) => {
                        ...
                        const otherPartyName = convo.otherParty?.fullName || `${convo.otherParty?.firstName} ${convo.otherParty?.lastName}`.trim() || "Unknown User";
                        const lastMessage = convo.lastMessageSnippet || "No messages yet..."; // Placeholder
                        const timestamp = convo.lastUpdatedAt ? new Date(convo.lastUpdatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "";

                        return (
                            <React.Fragment key={convo.contractId}>
                                <article
                                    className={`${styles.conversation} ${selectedContractId === convo.contractId ? styles.selectedConversation : ''}`}
                                    onClick={() => onSelectConversation(convo.contractId, convo.otherParty)} // Pass ID and other party info
                                    role="button"
                                    tabIndex={0}
                                    onKeyPress={(e) => (e.key === 'Enter' || e.key === ' ') && onSelectConversation(convo.contractId, convo.otherParty)}
                                >
                                    <img // Use standard img
                                        src={convo.otherParty?.profileImage || '/default-profile.png'}
                                        alt={otherPartyName}
                                    ...
                                    <div className={styles.messagePreview}>
                                        ...
                                        <p className={styles.lastMessage}>{convo.gigTitle || 'Contract Discussion'}</p>
                    ...
                    <p style={{ textAlign: 'center', padding: '20px', color: '#666' }}>No conversations found.</p>
                )}
            </div>
        </aside>
    );
