...
const LazyLoadingMonitor = ({
  ...
  useEffect(() => {
            ...
            images.forEach(img => {
              if (img.classList.contains('lazy-image')) {
                ...
                const onLoad = () => {
                  ...
                  console.debug(
                    `Lazy image loaded in ${loadTime.toFixed(2)}ms:`,
                    img.src
                  );
                };
    ...
    window.IntersectionObserver = function (...args) {
      observerCount++;
      console.debug(
        `IntersectionObserver created. Total active: ${observerCount}`
      );
      ...
      observer.disconnect = function () {
        observerCount--;
        console.debug(
          `IntersectionObserver disconnected. Total active: ${observerCount}`
        );
    ...
    const reportMetrics = () => {
      ...
      if (metrics.routeLoadTimes.size > 0) {
        ...
        const avgRouteTime =
          routeTimes.reduce((sum, [, data]) => sum + data.loadTime, 0) /
          routeTimes.length;
      ...
      if (metrics.imageLoadTimes.size > 0) {
        ...
        const avgImageTime =
          imageTimes.reduce((sum, time) => sum + time, 0) / imageTimes.length;
        ...
        console.log(
          `Success rate: ${((metrics.totalLazyLoads / (metrics.totalLazyLoads + metrics.failedLoads)) * 100).toFixed(1)}%`
        );
        console.groupEnd();
      }
