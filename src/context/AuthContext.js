...
export const AuthProvider = ({ children }) => {
        ...
        if (currentToken && currentToken !== 'null' && currentToken !== 'undefined') {
            ...
            try {
                ...
                if (response.data?.data?.user) {
                    ...
                    logger.debug("AuthContext: User fetched successfully:", response.data.data.user?._id);
                    return response.data.data.user; // Return the fetched user data
                } else {
                    logger.warn("AuthContext: Fetched user data missing in response from /users/me.");
                    logout(); // Call stable logout
                }
            } catch (error) {
                logger.error("AuthContext: Failed to fetch user with stored token:", error.response?.data || error.message);
    ...
    const contextValue = { authToken, user, login, logout, isLoading, refreshUser /*, logger */ };
...
export const useAuth = () => {
    ...
    if (context === undefined) {
        throw new Error('useAuth must be used within an AuthProvider. Make sure your component tree is wrapped.');
    }
    return context;
};