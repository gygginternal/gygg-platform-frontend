...
const OFF_PLATFORM_FILTERS = {
  ...
  emailPhrases: [
    ...
    'my email is',
    'email address',
    'etransfer to my email',
    'send to my email', // Canada-specific Interac phrasing
  ],
  ...
  offPlatformPhrases: [
    ...
    'pay me directly',
    'cheaper outside platform',
    'save on fees',
    'skip the fees',
    "don't pay here",
    'contact me outside',
    "let's connect elsewhere",
    'future deals outside this app',
    'no need to use this site',
    "i'll give you my details",
    'send money another way',
    'better deal off here',
    'cut out the middleman',
    'continue off the app',
...
const OFF_PLATFORM_REGEX = {
  ...
  bracketPattern: /\([^)]*@[^)]*\)/gi,
  spelledOutNumbers:
    /\b(?:one|two|three|four|five|six|seven|eight|nine|zero)\s+(?:one|two|three|four|five|six|seven|eight|nine|zero)\s+(?:one|two|three|four|five|six|seven|eight|nine|zero)\s+(?:one|two|three|four|five|six|seven|eight|nine|zero)\s+(?:one|two|three|four|five|six|seven|eight|nine|zero)\s+(?:one|two|three|four|five|six|seven|eight|nine|zero)\s+(?:one|two|three|four|five|six|seven|eight|nine|zero)\s+(?:one|two|three|four|five|six|seven|eight|nine|zero)\s+(?:one|two|three|four|five|six|seven|eight|nine|zero)\s+(?:one|two|three|four|five|six|seven|eight|nine|zero)\b/gi,
  spacedAppNames:
    /\b(?:whats?\s+app|pay\s+pal|cash\s+app|ven\s+moo|tele\s+gram|dis\s+cord)\b/gi,
};
...
const checkOffPlatformAttempts = text => {
  ...
  OFF_PLATFORM_FILTERS.emailDomains.forEach(domain => {
    ...
    if (regex.test(text)) {
      ...
      if (!categories.includes('direct_contact'))
        categories.push('direct_contact');
    }
  });
  ...
  if (urlMatches) {
    ...
    if (!categories.includes('external_links'))
      categories.push('external_links');
  }
...
const getOffPlatformViolationMessage = violation => {
  if (
    violation.includes('direct_contact') ||
    violation.includes('email') ||
    violation.includes('phone')
  ) {
    return 'Sharing personal contact information is not allowed. Please keep all communication within the platform.';
  }

  if (
    violation.includes('payment') ||
    violation.includes('crypto') ||
    violation.includes('banking')
  ) {
  ...
  if (
    violation.includes('social_media') ||
    violation.includes('social_handle')
  ) {
...
export const checkContent = (text, options = {}) => {
  ...
  return {
    ...
    severity:
      violations.length > 0
        ? violations.some(
            v => v.type === 'off_platform' || v.type === 'explicit'
          )
          ? 'high'
          : 'medium'
        : 'none',
...
export const getContentSafetyScore = text => {
  ...
  const explicitViolations = result.violations.filter(
    v => v.type === 'explicit'
  ).length;
  const patternViolations = result.violations.filter(
    v => v.type === 'pattern'
  ).length;
  const contextViolations = result.violations.filter(
    v => v.type === 'context-sensitive'
  ).length;
